...
Install the matching loader's JAR on both client and server. Forge's existing pedestal network protocol remains 9, and the armillary redesign requires no save migration or new runtime library. Existing placed pedestals receive the new appearance automatically. Future fuel integration can extend `PedestalManager.hasOperatingPower` without changing the field engine.
