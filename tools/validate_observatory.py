...
def validate(packaged=False):
    ...
    if packaged:
        ...
        for loader in ('fabric', 'forge', 'neoforge'):
            jar = ROOT / loader / f'build/libs/timestop-{loader}-{properties["minecraft_version"]}-{properties["mod_version"]}.jar'
            with zipfile.ZipFile(jar) as z:
                for path in DATA.rglob('*'):
                    if path.is_file() and ('observator' in str(path) or path.suffix == '.nbt'):
                        ...
                        resource = resource.replace('/structures/', '/structure/').replace('/loot_tables/', '/loot_table/')
