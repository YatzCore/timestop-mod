...
public record RewindBlocksPacket(ResourceLocation dimension, List<Entry> blocks) implements IClientboundPacket {
    public static final ResourceLocation ID = ResourceLocation.fromNamespaceAndPath(TimeStopMod.MOD_ID, "rewind_blocks");
    ...
    public RewindBlocksPacket(FriendlyByteBuf buf) {
        this(buf.readResourceLocation(), readEntries(buf));
    }
    ...
    private static List<Entry> readEntries(FriendlyByteBuf buf) {
        ...
        for (int i = 0; i < count; i++) {
            result.add(new Entry(buf.readBlockPos(), Block.stateById(buf.readVarInt())));
        }
        return result;
    }
    ...
    public void handleClient() {
        com.timestop.client.RewindBlockRenderer.begin(this);
    }
}
