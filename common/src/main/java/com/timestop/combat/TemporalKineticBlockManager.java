...
public class TemporalKineticBlockManager {
    ...
    public static void recordHit(Entity entity, Vec3 impulse, Player player) {
        ...
        if (volatileBlast) {
            com.timestop.platform.EntityDataHelper.getPersistentData(entity).putBoolean("VolatileStasis", true);
        }
    ...
    private static void applyDischargeSingle(ServerLevel level, Entity entity, KineticRecord record) {
        ...
        if (entity instanceof FallingBlockEntity fallingBlock) {
            ...
            if (record.puncherUuid != null) {
                com.timestop.platform.EntityDataHelper.getPersistentData(fallingBlock).putUUID("KineticPuncherUuid", record.puncherUuid);
            }
    ...
    public static void serverTick() {
        ...
        while (it.hasNext()) {
            ...
            if (block == null || !block.isAlive() || block.onGround() || block.getDeltaMovement().lengthSqr() < 0.04) {
                if (block != null && com.timestop.platform.EntityDataHelper.getPersistentData(block).getBoolean("VolatileStasis")) {
            ...
            if (block.level() instanceof ServerLevel level) {
                ...
                UUID puncherUuid = com.timestop.platform.EntityDataHelper.getPersistentData(block).hasUUID("KineticPuncherUuid") ? com.timestop.platform.EntityDataHelper.getPersistentData(block).getUUID("KineticPuncherUuid") : null;
                for (LivingEntity target : targets) {
                    if (target.isAlive()) {
                        ...
                        if (com.timestop.platform.EntityDataHelper.getPersistentData(block).getBoolean("VolatileStasis")) {
                            detonateVolatileBlast(block);
                            block.discard();
                            activeKineticBlocks.remove(ref);
                            break;
