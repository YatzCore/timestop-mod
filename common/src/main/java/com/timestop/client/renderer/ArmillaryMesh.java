        ...
        for (Part part:parts) {
            ...
            for(int face=0;face<v.length;face+=32) {
                ...
                for(int i=face;i<face+32;i+=8) {
                    ...
                    buffer.addVertex(position.x,position.y,position.z).setColor(tint,tint,tint,1).setUv(v[i+3],v[i+4]).setOverlay(overlay).setLight(illumination).setNormal(normal.x,normal.y,normal.z);
                }
            }
        }
    }
