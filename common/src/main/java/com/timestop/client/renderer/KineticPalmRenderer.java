...
public class KineticPalmRenderer {
    ...
    public static void renderLevel(PoseStack poseStack, Camera camera, float partialTick) {
        ...
        var bullets = mc.level.getEntitiesOfClass(Projectile.class, mc.player.getBoundingBox().inflate(40),
                p -> com.timestop.platform.EntityDataHelper.getPersistentData(p).getBoolean("KineticPalmCaptured")
                        && com.timestop.platform.EntityDataHelper.getPersistentData(p).getInt("NeoWakeAge") < 12);
        ...
        var matrix = poseStack != null ? poseStack.last().pose() : new Matrix4f();
        Vec3 camPos = camera.getPosition();
        BufferBuilder buffer = tessellator.begin(VertexFormat.Mode.QUADS, DefaultVertexFormat.POSITION_COLOR);
        for (Projectile bullet : bullets) {
            float age = com.timestop.platform.EntityDataHelper.getPersistentData(bullet).getInt("NeoWakeAge") + partialTick;
            ...
            Vec3 position = bullet.getPosition(partialTick).subtract(camPos);
    ...
    public static void renderHand(InteractionHand hand, PoseStack pose) {
        Minecraft mc = Minecraft.getInstance();
