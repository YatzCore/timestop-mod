...
public final class RewindBlockRenderer {
    ...
    private static void draw(BlockState state, PoseStack pose, float scale, float alpha, int light) {
        ...
        MultiBufferSource source = type -> new FadedVertex(buffers.getBuffer(RenderType.entityTranslucent(TextureAtlas.LOCATION_BLOCKS)), alpha);
    ...
    private record FadedVertex(VertexConsumer target, float alpha) implements VertexConsumer {
        @Override public VertexConsumer vertex(double x, double y, double z) { target.vertex(x, y, z); return this; }
        @Override public VertexConsumer color(int r, int g, int b, int a) { target.color(r, g, b, (int)(a * alpha)); return this; }
        ...
        @Override public void defaultColor(int r, int g, int b, int a) { target.defaultColor(r, g, b, (int)(a * alpha)); }
