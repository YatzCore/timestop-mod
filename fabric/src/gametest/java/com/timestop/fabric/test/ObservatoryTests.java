...
public class ObservatoryTests implements FabricGameTest {
    ...
    public void vanillaTemplateCommandsPreserveLootAndPedestals(GameTestHelper h) {
        ...
        for(String variant:List.of("highland","forest","cherry","floral","windswept","acropolis")) {
            ...
            int result=PortTestSupport.command(level.getServer().getCommands(), level.getServer().createCommandSourceStack().withSuppressedOutput(),
            ...
            h.assertTrue(chest!=null && chest.saveWithoutMetadata(h.getLevel().registryAccess()).getString("LootTable").equals("timestop:chests/observatory_archive"),"Command placement must retain deferred archive loot");
        }
        h.succeed();
    }
            ...
            for(String variant:List.of("highland","forest","cherry","floral","windswept","acropolis")) for(Rotation rotation:Rotation.values()) {
                ...
                for(var entry:raw.getList("blocks",Tag.TAG_COMPOUND)) {
                    ...
                    if(level.getBlockEntity(pos) instanceof PedestalBlockEntity pedestal) {
                        ...
                        expected.useItemOn(player.getMainHandItem(),level,player,InteractionHand.MAIN_HAND,new BlockHitResult(Vec3.atCenterOf(pos),Direction.UP,pos,false));
                        ...
                        var persisted=pedestal.saveWithoutMetadata(h.getLevel().registryAccess()); pedestal.loadWithComponents(persisted, h.getLevel().registryAccess());
                    ...
                    if(level.getBlockEntity(pos) instanceof ChestBlockEntity chest) {
                        chests++;
                        h.assertTrue(chest.saveWithoutMetadata(h.getLevel().registryAccess()).contains("LootTable"),"Unopened chest retains persistent loot reference: "+variant+" "+rotation+" "+local+" "+chest.saveWithoutMetadata(h.getLevel().registryAccess()));
                        chest.unpackLootTable(null); chest.clearContent();
                        var persisted=chest.saveWithoutMetadata(h.getLevel().registryAccess()); chest.loadWithComponents(persisted, h.getLevel().registryAccess());
    ...
    private static void validatePalette(GameTestHelper h,String variant) {
        try(var input=ObservatoryTests.class.getResourceAsStream("/data/timestop/structure/ruined_observatory/"+variant+".nbt")) {
            var nbt=NbtIo.readCompressed(input, NbtAccounter.unlimitedHeap());
            for(var tag:nbt.getList("palette",Tag.TAG_COMPOUND)) {
                var state=(CompoundTag)tag; var id=ResourceLocation.parse(state.getString("Name"));
    ...
    public void archiveLootGuaranteesAndDistribution(GameTestHelper h) {
        var table=h.getLevel().getServer().reloadableRegistries().getLootTable(net.minecraft.resources.ResourceKey.create(Registries.LOOT_TABLE, ResourceLocation.fromNamespaceAndPath("timestop","chests/observatory_archive")));
    ...
    public void terrainAcrossSeeds(GameTestHelper h) {
        ...
        for(long seed:new long[]{42,16310624,8675309}) {
            ...
            for(int x=-4;x<=4;x++) for(int z=-4;z<=4;z++) {
                ...
                for(String variant:List.of("highland","forest","cherry","floral","windswept","acropolis")) {
                    var structure=access.registryOrThrow(Registries.STRUCTURE).get(ResourceLocation.fromNamespaceAndPath("timestop","ruined_observatory_"+variant));
