...
public class RewindAllowedCommandTests implements FabricGameTest {
    ...
    public void defaultStateIsOn(GameTestHelper h) {
        h.assertTrue(TimeStopSavedData.load(new CompoundTag(), h.getLevel().registryAccess()).isRewindModeAllowed(),
    ...
    public void commandTogglingAndPermissions(GameTestHelper h) throws Exception {
        ...
        try {
            ...
            h.assertTrue(!TimeStopSavedData.load(saved, h.getLevel().registryAccess()).isRewindModeAllowed(), "Disabled state must survive save/load");
