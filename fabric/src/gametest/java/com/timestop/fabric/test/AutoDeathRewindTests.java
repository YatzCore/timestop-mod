...
public class AutoDeathRewindTests implements FabricGameTest {
    ...
    public void commandIsAdminOnlyAndSettingPersists(GameTestHelper h) throws Exception {
        ...
        try {
            h.assertTrue(!TimeStopSavedData.load(new net.minecraft.nbt.CompoundTag(), h.getLevel().registryAccess()).isAutoDeathRewind(),"New worlds must default to OFF");
            ...
            h.assertTrue(TimeStopSavedData.load(settings.save(new net.minecraft.nbt.CompoundTag(), h.getLevel().registryAccess()), h.getLevel().registryAccess()).isAutoDeathRewind(),"Enabled setting must survive save/load");
            ...
            h.assertTrue(!TimeStopSavedData.load(settings.save(new net.minecraft.nbt.CompoundTag(), h.getLevel().registryAccess()), h.getLevel().registryAccess()).isAutoDeathRewind(),"Disabling must persist too");
