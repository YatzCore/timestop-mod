...
public class PedestalVisualQa implements ClientModInitializer {
    ...
    private void tick(Minecraft mc) {
        ...
        if (phase==0 && mc.screen instanceof TitleScreen && mc.getOverlay()==null) {
            ...
            if (Files.exists(mc.gameDirectory.toPath().resolve("saves/PedestalQA/level.dat"))) mc.createWorldOpenFlows().openWorld("PedestalQA", () -> mc.setScreen(new TitleScreen()));
            else {
                ...
                mc.createWorldOpenFlows().createFreshLevel("PedestalQA",new LevelSettings("Pedestal QA",GameType.CREATIVE,false,Difficulty.PEACEFUL,true,rules,WorldDataConfiguration.DEFAULT),
                        new WorldOptions(42,false,false),access -> access.registryOrThrow(Registries.WORLD_PRESET).getHolderOrThrow(WorldPresets.FLAT).value().createWorldDimensions(), mc.screen);
            }
        }
