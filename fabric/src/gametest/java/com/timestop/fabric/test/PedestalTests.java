...
public class PedestalTests implements FabricGameTest {
    ...
    public void bulletSweepRetainsExplicitFlowExemption(GameTestHelper h) {
        ...
        Vec3 center=Vec3.atCenterOf(h.absolutePos(new BlockPos(5,5,5)));
        ...
        var arrow=new Arrow(h.getLevel(),center.x-10,center.y-.25,center.z); arrow.setOwner(player); arrow.setDeltaMovement(20,0,0);
        try {
    ...
    public void fastBulletStopsAtEntryAndResumes(GameTestHelper h) {
        ...
        var arrow=new Arrow(h.getLevel(),center.x-10,center.y-.25,center.z);
    ...
    public void bulletSweepRespectsStrongerOverlap(GameTestHelper h) {
        ...
        var arrow=new Arrow(h.getLevel(),center.x-10,center.y-.25,center.z); arrow.setDeltaMovement(20,0,0);
        try {
    ...
    public void bulletMissAndEarlierWallStayNative(GameTestHelper h) {
        ...
        var freeze=new TemporalBubble(UUID.randomUUID(),UUID.randomUUID(),h.getLevel().dimension(),center,2,TimeMode.TIME_STOP,0,WatchTier.DIAMOND,null,0,Set.of()).stationary(true);
        TemporalBubbleManager.putStationaryBubble(h.getLevel(),freeze,null);
        var arrow=new Arrow(h.getLevel(),center.x-10,center.y-.25,center.z+3); arrow.setDeltaMovement(20,0,0);
    ...
    public void furnaceSmeltingAcceleratesAndFreezes(GameTestHelper h) {
        ...
        h.runAfterDelay(50,() -> {
            ...
            int progress=accelerated.saveWithoutMetadata().getShort("CookTime");
            p.configure(TimeMode.TIME_STOP,2); PedestalManager.serverTick();
            h.runAfterDelay(10,() -> {
                try {
                    h.assertTrue(accelerated.saveWithoutMetadata().getShort("CookTime")==progress,"Stasis freezes real furnace cooking progress");
                    h.succeed();
                } finally {
    ...
    public void breakingDropsExactlyOneUnmodifiedWatch(GameTestHelper h) {
        ...
        p.getWatch().getOrCreateTag().putString("PedestalTest",marker); p.getWatch().setDamageValue(31);
        ...
        var drops=h.getLevel().getEntitiesOfClass(net.minecraft.world.entity.item.ItemEntity.class,new net.minecraft.world.phys.AABB(p.getBlockPos()).inflate(5),
                item -> item.getItem().hasTag() && marker.equals(item.getItem().getTag().getString("PedestalTest")));
    ...
    public void rewindReconcilesFieldAndDoesNotDropWatch(GameTestHelper h) {
        ...
        try {
            p.configure(TimeMode.TIME_STOP,2); power(h,p,true);
            var original=p.saveWithFullMetadata();
            p.configure(TimeMode.FAST_FORWARD,1); PedestalManager.serverTick();
            var frame=new com.timestop.core.rewind.data.TickFrame(0);
            frame.addBlockEntityDelta(com.timestop.core.rewind.data.BlockEntityDelta.create(h.getLevel().dimension(),p.getBlockPos(),ModPedestals.ENTITY.getId(),original,p.saveWithFullMetadata()));
            ...
            var marker=UUID.randomUUID().toString(); p.getWatch().getOrCreateTag().putString("PedestalTest",marker);
            var placement=new com.timestop.core.rewind.data.TickFrame(1);
            placement.addBlockDelta(com.timestop.core.rewind.data.BlockDelta.create(h.getLevel().dimension(),p.getBlockPos(),Blocks.AIR.defaultBlockState(),p.getBlockState(),null,p.saveWithFullMetadata()));
            ...
            h.assertTrue(h.getLevel().getEntitiesOfClass(net.minecraft.world.entity.item.ItemEntity.class,new net.minecraft.world.phys.AABB(p.getBlockPos()).inflate(5),
                    item -> item.getItem().hasTag() && marker.equals(item.getItem().getTag().getString("PedestalTest"))).isEmpty(),"Rewind removal must not generate a duplicate watch drop");
        } finally { clean(h,p); }
        h.succeed();
    }
    ...
    public void sourceLifecycleAndWatchNbt(GameTestHelper h) {
        ...
        try {
            p.getWatch().setDamageValue(27); p.getWatch().getOrCreateTag().putString("CustomNameTest","Original");
            p.configure(TimeMode.DECELERATION_FIELD,4); power(h,p,true);
            CompoundTag saved=p.saveWithFullMetadata(); UUID id=p.getFieldId();
            p.setRemoved(); h.assertTrue(TemporalBubbleManager.getBubble(id)==null,"Unload hook removes field");
            p.clearRemoved(); p.load(saved); PedestalManager.serverTick();
            h.assertTrue(TemporalBubbleManager.getBubble(id)!=null,"Reload restores powered source");
            h.assertTrue(p.getWatch().getDamageValue()==27 && p.getWatch().getTag().getString("CustomNameTest").equals("Original"),"NBT round trip");
            ...
            p.load(restored); PedestalManager.serverTick();
    ...
    public void playerSettingAndPermissions(GameTestHelper h) {
        ...
        try {
            ...
            CompoundTag tag=p.saveWithFullMetadata(); tag.putUUID("Owner",UUID.randomUUID()); p.load(tag);
            ...
            int denied=commands.performPrefixedCommand(stranger.createCommandSourceStack().withPermission(0),"timestop pedestal affectplayers false");
            h.assertTrue(denied==0 && TimeStopSavedData.get().pedestalsAffectPlayers(),"Non-operator cannot change policy");
            commands.performPrefixedCommand(h.getLevel().getServer().createCommandSourceStack(),"timestop pedestal affectplayers false");
            h.assertTrue(TemporalBubbleManager.getBubble(p.getFieldId()).canEntityAct(stranger),"Command updates existing fields");
            h.assertFalse(TimeStopSavedData.load(TimeStopSavedData.get().save(new CompoundTag())).pedestalsAffectPlayers(),"Policy saved with world");
            h.assertTrue(TimeStopSavedData.load(new CompoundTag()).pedestalsAffectPlayers(),"Old worlds default true");
            stranger.discard();
    ...
    public void decelerationWithoutOnlineOwner(GameTestHelper h) {
        ...
        try {
            ...
            Arrow arrow=new Arrow(h.getLevel(),p.getBlockPos().getX()+.5,p.getBlockPos().getY()+1.5,p.getBlockPos().getZ()+.5);
