...
public class RewindRuneIntegrationTests implements FabricGameTest {
    ...
    public void consumptionFollowsExactRuneAndSurvivesCompletion(GameTestHelper h) {
        ...
        container.getList("Items", 10).add(AbstractWatchItem.getSocketedRune(historical.inventory().get(40)).save(new net.minecraft.nbt.CompoundTag()));
        var sanitized = RewindRuneManager.restorationNbt(container);
        h.assertTrue(ItemStack.of(sanitized.getList("Items", 10).getCompound(0)).isEmpty(), "Container restoration must not resurrect spent rune identities");
        h.assertTrue(!ItemStack.of(container.getList("Items", 10).getCompound(0)).isEmpty(), "Sanitizing restoration must not mutate stored timeline NBT");
        player.discard(); h.succeed();
    }
    ...
    public void spentRuneCannotReturnToItsHistoricalChest(GameTestHelper h) {
        ...
        var historicalTag = chest.saveWithFullMetadata();
        ...
        frame.addBlockEntityDelta(com.timestop.core.rewind.data.BlockEntityDelta.create(level.dimension(), pos,
                new net.minecraft.resources.ResourceLocation("minecraft:chest"), historicalTag, null));
